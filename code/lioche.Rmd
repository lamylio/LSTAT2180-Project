...
alpha = 0.05
theta = pgeom(2, 0.5, lower.tail = F) # >2 = 0.125

# J'ai ajouté une correction de bias à l'estimateur de p, et à T.param
p.estim = function(obs){ph = (1/(mean(obs)+1)); ph-(ph*(1-ph))/length(obs)}
T.param = function(obs){return (round((1 - p.estim(obs) )^3, 4))}
...
sample.p = p.estim(sample)

sample.T.param = T.param(sample)
sample.T.nonparam = T.nonparam(sample)

# ---
export.list = c("opt.digits", "sample", "n", "p", "M", "alpha", "theta", "sample.mu", "sample.p", "p.estim", "T.param", "T.nonparam", "sample.T.param", "sample.T.nonparam", "T.boot")
...
HT.theta.normal = function(this.theta, this.sample){
  bootSample = T.boot(T.nonparam, this.sample=this.sample)
  
  check = abs(
    (mean(bootSample)-this.theta)
    /
    (sd(bootSample)*sqrt(length(this.sample)))
    ) <= 1.96
  ifelse(check, "H0: Non-rejected", "H0: Rejected")
}
    ...
    coverage.loc = parLapply(cl, 1:B, function(i){
     ...
     # ---
      ...
      all = list(
        ...
        CI.asym.nonparam(newSample.T.nonparam, var(newSample.boot.T)),
